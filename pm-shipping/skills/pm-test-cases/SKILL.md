...
> 流程内落盘路径 `prd/test/{日期}-{项目}-测试用例-V{版本}.md`（流程门禁按 glob 匹配，命名带产品名/日期是正常的）。
...
SRS／PRD 里若有**业务对象状态机表**（订单、审核、退款、工单、提现、内容发布；
...
- **非法操作拦截那一类最容易整块漏**，而它正是线上事故高发区（绕过界面直接调接口改已完结的单据）。
  它属于「权限测试」还是「异常测试」不重要，**关键是别因为界面上点不到就不写**。
- 状态机里的**每个状态至少被一条用例覆盖到**，终态要验证"确实不能再流转"。
- 表里没有的流转要**当成缺陷提出来**（"驳回后能否重新提交"没写），不要自己假设一个补上。
