...
本 Skill 是 pm-master 13 阶段流程的**数据分析深度档（默认档 pm-product-metrics）；不是 13 阶段中的固定阶段，按需挂载**，也可单点直接调用；由 `pm-master` 总控统一路由。

- **上游**：`pm-tracking-spec-writer`（埋点数据就绪后才有得分析）、`pm-survey-designer`（问卷回收数据）
- **下游**：`pm-experiment-designer`（归因产生的假设交给实验验证）、`pm-prioritization-engine`（决策建议转需求排序）
