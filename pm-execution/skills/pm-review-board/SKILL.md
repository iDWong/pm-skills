...
评审前先确认（或从上下文推断）本次评审的目的，三档标准不同：

| 档位 | 场景 | 评审口径 |
|------|------|---------|
| 快速过筛 | 需求还在发散期，只想找大问题 | 只报阻断项 + 结构性问题，忽略细节；每角色 ≤2 条意见 |
| 标准评审（默认） | 进评审会前的预演 | 完整六角色流程，全量分级 |
| 上线前终审 | 提测/上线前最后一道 | 在标准评审之上，法务/运营/测试三角色提权：任何合规、回滚、验收标准问题一律升为阻断项 |

用户没说时默认标准评审，但材料里出现"明天上线""已提测"字样时主动确认是否切终审档。
...
每条评审意见必须同时满足三个条件，缺一即删：

1. **指得到位置**：引用材料的具体章节/段落/界面元素，禁止"整体感觉不够清晰"
2. **说得清后果**：这个问题不解决，会在哪个环节炸（开发返工/测试无法验收/上线事故/合规风险）
3. **给得出动作**：修改建议要具体到"补充什么内容/改成什么方案"，禁止"建议优化""建议完善"

反例："建议加强异常处理" ❌ → 正例："3.2 提交订单未定义支付超时后的订单状态，建议补充：超时 30 分钟自动关单并释放库存，用户侧提示重新下单" ✅
...
材料重大（核心业务改造、大额投入、不可逆决策）或用户明确要求"角色之间不要互相影响"时，可 spawn 多个 subagent，每个只带一个角色的审查要点独立评审，主 agent 只做冲突归并、风险分级和结论输出。好处：角色视角不串味，问题发现率更高。代价：更慢。默认不启用，重大材料时向用户提议。
