...
description: |
  ...
  触发条件：用户提到"路线图"、"版本规划"、"roadmap"、"里程碑"、"季度计划"、"版本排期"、"项目排期"、"依赖梳理"、"风险预案"、"缓冲方案"、"release plan"、"OKR拆解"等关键词。
  也适用于：用户提供季度目标/团队产能/依赖信息要求排期；用户要求将战略目标拆解为可执行的里程碑；用户要求评估项目风险和缓冲方案；用户要求制定跨团队协作的版本计划。
  典型输入：季度目标 + 团队产能 + 依赖方信息 + 约束条件。
  流程位置：`pm-master` 阶段 3 的**深度档**（默认档是 `pm-roadmap`）——两者共享「路线图 / roadmap / 版本规划」这组触发词是**设计如此**，
...
本 Skill 是 pm-master 13 阶段流程的**阶段3 深度档（默认档 pm-roadmap）**，也可单点直接调用；由 `pm-master` 总控统一路由。

- **上游**：`pm-prioritization-engine`（Now/Next/Later 排序结果是理想输入）
- **下游**：`pm-postmortem-writer`（版本结束后复盘，路线图的「每阶段成功指标」即复盘的对照基线）
