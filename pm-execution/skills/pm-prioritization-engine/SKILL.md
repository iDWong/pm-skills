...
description: |
  对需求/功能/项目进行多维度优先级排序，输出版本排期建议。当用户说"帮我排优先级"、"这些需求怎么排"、"用 RICE 打分"、"ICE 评分"、"Kano 分析"、"成本收益分析"、"功能排期"、"哪些先做哪些后做"，
...
排序做完后，检验它的脆弱程度——评审会上最容易被挑战的就是这里：

1. **找关键参数**：对 Top 3 需求，逐一问"哪个评分维度变动会让它掉出 Top 3？变动多少？"
2. **标注脆弱排名**：某需求的排名依赖一个信心 ≤50% 的估算时，标注 ⚠️ 并写明"该排名取决于 [假设X]，建议先花最小成本验证"
3. **输出稳健性结论**：一句话总结，例："Top 3 中 R1/R2 在参数扰动 ±30% 内排名不变，R4 对触达量估算高度敏感——它的排位本质上是在赌 [假设]"

### 高分歧需求的价值公式复核

模型间分歧大的需求，用俞军的用户价值公式做第三方裁决：**用户价值 = 新体验 − 旧体验 − 替换成本**。三问：用户现在怎么解决（旧体验基线）？新方案好多少（增量而非绝对值）？用户迁移要付出什么（学习/数据/习惯成本）？若装有 `pm-advisor-yujun`，可加载它做完整分析。公式算出来接近零或为负 → 无论 RICE 分多高都建议降级，并说明理由。
...
| 9 | 敏感性分析已做 | Top 3 排名的脆弱点已标注 |
| 10 | 高分歧需求已用价值公式复核 | 分歧不是靠平均分掩盖，而是被裁决 |
...
本 Skill 是 pm-master 13 阶段流程的**阶段2 深度档（默认档 pm-feature-prioritization）**，也可单点直接调用；由 `pm-master` 总控统一路由。

- **上游**：需求池可来自 `pm-review-board`（评审通过的需求）、`pm-analytics`（数据洞察转需求）、`pm-competitor-deconstructor`（竞品差距转需求）、`pm-postmortem-writer`（复盘行动项）
- **下游**：`pm-roadmap-planner`（排序结果 + 产能 → 路线图）；高分歧需求可转 `pm-advisory-board` 用俞军价值公式复核
