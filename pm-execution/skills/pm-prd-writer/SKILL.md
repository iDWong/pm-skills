...
name: pm-prd-writer
description: |
  把模糊需求转化为可评审的产品需求文档（PRD）。当用户说"帮我出PRD"、"写个产品需求文档"、"这个功能怎么写需求"、"我有个想法想落地"、"把这个需求整理成文档"、"需求评审要用的PRD"，或者用户描述了一段功能但没有结构化时，使用这个 Skill。
  也适用于：用户上传了原始需求描述/会议纪要/聊天截图并要求整理成PRD；用户说"PRD"、"产品需求"、"需求还很模糊"等关键词；用户要求对已有PRD进行补全、优化、查漏补缺。
  下游接力：要字段级可开发的规格版 PRD（五形态判定、每页配线框图、UX 规范）→ 本技能澄清与补漏完成后转 `pm-prd-spec`。
...
完整的 PRD 文档，**默认输出 `.md`，不生成 docx**（见文末「输出格式」；用户明确要 `.docx` 才转）。包含：
- 版本记录表
- 完整的五大章节
...
- **默认输出 `.md` 格式**（适合在线协作和评审）；**不主动生成 docx，也不要问「要不要导 Word」**
- 仅当用户**原话开口**要 `.docx` / Word / 评审稿文件时才转：先读取 docx Skill（如果可用），按 docx 规范输出
