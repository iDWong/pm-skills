...
**叫法**：Step 8 的交付物——`design-system/` 下那批可点 HTML——正式名是 **UI/UX设计稿**，上下文清楚时简称**设计稿**（两者同义，标题与首次出现用全称）；PRD 正文里嵌的那张静态图叫**页面原型图**。文档、README、交付说明与产物页面上的可见文案，一律按这两个词写。
...
| 项 | 要求 |
| --- | --- |
| 多端与一致性 | 覆盖多端时：平台差异走**独立页面 + 独立卡片，不切帧**，只允许导航 / 返回 / 权限弹窗 / 通知 / 深色 / 折叠屏平板六类；HANDOFF 增第 9 节「多端适配」，另交 `CONSISTENCY.md`。八端规则与判定表见 `ui-ux-pro-max/references/platform-rules.md`，设计系统内容规格见同目录 `design-system.md` |
| 信息架构 | HANDOFF 第 4 节先给 **Mermaid 站点地图 + 分级页面清单**；站点地图节点数 = 页面清单条数 = 产物 HTML 数（平台变体计一条并注明变体数），对不上就是有页面漏建或漏进架构 |
| 清单反查九条 | 文档之外，设计稿**自己**要做到：组件八态真到得了、官网四断点真重排、动效走令牌且带 reduced-motion、`tokens.json` 与页面 CSS 变量一一对应、文案集中管理、输入框必带 `data-rule`、列表有极限样本、图标位图导出 `assets/`、HANDOFF 第 2–4 节由脚本生成。契约附机检脚本，输出必须为 OK |
| 工程师对接清单 | 必交 `HANDOFF.md`（八节：交付信息头 / 设计令牌 / 组件清单 / **页面与状态矩阵** / 交互细节 / 文案清单 / 资源交付 / 协作验收）+ `tokens.json`。原则：**能给数值不给描述，能给规则不给感觉**。第 2–4 节由 build 自动生成，状态矩阵里每一态都要写明走到它的演示数据路径；末尾十条高频漏项自查全绿才算交付 |
| 落盘位置 | `design-system/<项目slug>/`（项目根下，与设计令牌母版 `MASTER.md` 同树；旧根 `Prototype/` 已废除，存量读时兼容、不主动搬家）；生成器落其 `_src/`，产物由 `python3 _src/build.py` 构建，手改产物会被覆盖 |
...
| 索引页页头 | badge + 标题 + **出稿口径卡**（通栏磨砂卡，常驻正文不做成开关浮层；标题写 `出稿口径 · 依据 {PRD 文件名}`，8–10 条评审真会问的口径）+ **KPI 胶囊行**（写评审真看的数：`页面 N` / `交互流程 N` / 弹窗或浮层数 / 视口或画布尺寸 / **`依赖 0`**；**每个数都要是实测值，且不放 PRD 版本或日期胶囊**）+ 主题与语言切换 + **跨墙互跳胶囊**。H1 里不挂计数 |
...
| 工具条位置 | **墙内不注入任何工具条**；需求标注开关与「重置演示进度」**只在全屏页**，固定右下角，样式一眼可辨为非产品 UI（caption 写 `PROTOTYPE ONLY`）。「只在全屏页」是字面意思——**墙页面自己也不挂 devbar**，因此墙上不能有任何要点开关才看得到的内容（墙的出稿口径走页头那张卡） |
| 弹窗与浮层 | **PRD 点名的每一个都要建、且都点得到**：后台各页「编辑 / 危险操作」小节的每个表单与确认层，C 端页面清单「关联浮层」列（该列会合并计数，按展开后的层数做）。用户触发的接真实入口（表格「操作」列、页面按钮），**不许加一排「打开 XX 弹窗」的凑数按钮**；自动触发的（注册墙、权限层、一次性动效）**按真实触发条件自动弹**（产物写 `data-auto`，首次进入用 `localStorage` 记标志），**不许挂帧**。build 后跑「死按钮 / 孤层」自查，两项都必须为 0。细则见契约「弹窗与浮层」 |
...
| 完整流程（取代出稿帧） | **取消出稿帧**：每页声明 `flows=[...]`，build 汇总出 `FLOWS.md`（流程名 / 起点 / 步骤 / 分支 / 终态 / 出处）。流程态由真实动作 + 确定性延时推进，**成功与失败两条分支都要走得到**（分支靠演示数据定死，禁 `Math.random()`）；数据态（空 / 加载 / 错误 / 无权限 / 额度用尽 / 已下架）各给一条演示数据入口，摆在页面正文的真实位置而非一排「打开 XX 状态」的按钮；多态节点写 `data-state="…"`，状态名用业务语义禁止 `state1/state2`；产物里出现 `data-setframe` 即不合格 |
...
上面那份自查只解决「交付形态对不对」。**设计稿能不能支撑业务流程跑通，是另一件事**，
判据与清单在 `references/prototype-review.md`，四阶段逐层递进：

| 阶段 | 谁做 | 查什么 | 准出 |
| --- | --- | --- | --- |
| 机检 | 出稿方 | 三组脚本：① 死按钮 / 孤层 / **残留切帧** ② 流程完整性（`data-setframe` 为 0、无 `Math.random(`、每条 `data-flow` 有步骤）③ 清单反查九条（令牌同源 / 动效令牌 / reduced-motion / `data-rule`）；另加页面闭环（死链 / 无入链 / 无出链） | 全部输出 OK / 为 0 |
...
- 想出 **UI/UX设计稿**（触发词：设计稿 / 高保真原型 / 可点原型 / 交互原型 / 预览墙）→ 走本技能 **Step 8**。四件事别搞混：
  ...
  - **产物**：落 `design-system/<项目slug>/`，**共三张预览墙**：移动墙（APP / H5 / 小程序 **共用一张**，393×852，卡片按形态分组）、官网墙（1280×900）、后台墙（1440×900）。点预览卡即进全屏；墙内不注入工具条，需求标注开关与「重置演示进度」只在全屏页右下角。**不做出稿帧** —— 交付的是完整流程的动态交互设计稿，所有状态与分支都靠真实操作走到，并附 `FLOWS.md` 流程清单与 `HANDOFF.md` 工程师对接清单（八节 + `tokens.json`）。
