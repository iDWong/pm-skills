...
- 一个项目**可能横跨多个形态**（C 端 APP + 运营后台）。**默认按形态拆成多份 PRD**，在每份开头写明"本文档只覆盖 X 形态，Y 形态见另一份"。
  - **唯一可以合成一份的情形**：多个端是**同一套业务对象与流程的不同面**（例：创作者发文 → 用户阅读 → 管理员审核，三端咬合同一个"作品"对象），
    ...
    且开篇「美术与交互」按端各写一段。**判据是业务对象是否共享，不是"都是我们的产品"**——
...
| **美术与交互** | 按端各一段（整体布局 / 色彩 / 核心页面 / **一句话**）+ 一段通用交互 | Step 3 出设计系统后写，是它的自然产物 |
| **本期不做** | 逐条列明确排除的功能，每条写明「本期暂不开发」 | **Step 2 骨架确认时就要问定**，别拖到最后 |

「本期不做」是范围边界，**不是免责声明**：没写进来的争议最后都会变成返工。
...
**形态特有的全局章节由 profile 规定**——后台是权限模型与操作日志，APP 是权限申请/推送/版本更新/离线，小程序是授权登录/订阅消息/审核合规，官网是 SEO/CMS 可维护性。**五个形态现在都带「多语言（如有）」与「本期不做」**，多语言要写清覆盖范围（通常只覆盖界面固定文本，不含用户产生的内容）与配置形式。
