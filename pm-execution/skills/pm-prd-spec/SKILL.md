...
- 一个项目**可能横跨多个形态**（C 端 APP + 运营后台）。**默认按形态拆成多份 PRD**，在每份开头写明"本文档只覆盖 X 形态，Y 形态见另一份"。
  - **唯一可以合成一份的情形**：多个端是**同一套业务对象与流程的不同面**（例：创作者发文 → 用户阅读 → 管理员审核，三端咬合同一个"作品"对象），
    拆开会让一条业务线横跨三份文档、改一处要同步三处。这时合成一份，但**必须按端分一级章节**（`4 用户端H5` / `5 创作者平台` / `6 管理后台`），
    且开篇「设计与交互」按端各写一段。**判据是业务对象是否共享，不是"都是我们的产品"**——
    共享才合，不共享（例：C 端商城 + 内部 HR 系统）照旧拆。
  - 合成一份时形态后缀取主形态或写"全平台"，并在「系统简介」里点明包含哪几个端。
- 用户要的是 **SRS** → 转 `req-doc`。
...
9. **每个 🔴 页面必须有「边界条件」与「验收标准」两个收尾小节。** 边界条件五问逐条答（内容空/超长、网络异常、无权限、并发改同一条、格式不符），不适用也要写明理由；验收标准 2–5 条、句式「做了什么 → 看到什么」、至少一条失败路径。六态写的是这一屏长什么样，边界条件写的是数据与并发出意外时怎么办，**两件事不能互相顶替**。
...
页面清单统一给一张表作为后续章节目录，**每页必须带优先级**：

| 编号 | 页面 | 优先级 | 类型 | 入口 | 关键交互 | 关联浮层 |
| --- | --- | --- | --- | --- | --- | --- |
| 3.1 | 内容详情页 | 🔴 | 详情页 | 首页Tab → 卡片点击 | 上滑加载下一条、双击点赞 | 分享面板、举报弹窗 |
| 3.5 | 历史记录页 | 🟡 | 列表页 | 我的 → 最近阅读 | 分页加载 | — |
...
| 标记 | 含义 | Step 4 怎么写 |
| --- | --- | --- |
| 🔴 **核心** | 本期 MVP 必须有，缺了产品跑不通 | **完整展开** profile 规定的全部小节 + 原型图 |
| 🟡 **重要** | 本期之后迭代 | **只在本清单留名 + 一两句说明**，不写正文小节，不画原型图 |
| ⚪ **未来** | 方向上要做，时间未定 | 同 🟡 |

**范围闸门（Step 4 动笔前必须过）**：清单确认后先算一句话给用户——
「本期共 N 个 🔴 页面，先只展开这些；🟡/⚪ 只在清单留名，是否下一版再写？」**得到答复再动笔**。
没有这道闸门，页面一多文档就会失控地膨胀，而 80% 的篇幅花在本期根本不做的页面上。

- 优先级来源：阶段3 路线图 / 用户明确说明 / `prd/planning/roadmap-*.md`。三者冲突时**列出冲突请用户裁定**，不要自己挑一个。
- 用户要全量展开时照做，但要说明代价（篇幅与评审时长）。
- 本期不做的页面同时写进「0.2 产品边界」，**两处对得上**——清单里标 🟡/⚪ 的，产品边界里应有对应条目。
...
| N+1 | **界面与交互规范** | 来自 ui-ux-pro-max 检索，`维度 \| 规范 \| 依据` 三列 |
| N+2 | **边界条件** | 五问逐条答：内容空/超长、网络异常、无权限、**并发改同一条**、格式不符；不适用写明「不适用（理由）」，不要整条省略 |
| N+3 | **验收标准** | 2–5 条可勾选，句式「做了什么 → 看到什么」，**至少一条失败路径**；会被 `pm-test-cases` 直接转用例 |

**只写 🔴 页面。** Step 2 标 🟡/⚪ 的页面在本步**不展开**，清单里的名字与一两句说明就是它们的全部内容。
用户要全量时照做，但先说明篇幅代价。

后三个小节的写法与判据见 `references/doc-style.md`（「边界条件五问」「验收标准」）。
**有写入的页面（新建/编辑/提交）还要给 T3b 数据规范表**——T3 给前端做表单，T3b 给后端建表，
两张表的必填与唯一性必须一致。纯展示页不需要。
...
**开篇三节（落在文档最前面，不是附录，一个都不能少）**——写法与判据见 `references/doc-style.md`「开篇三节」：

| 节 | 写什么 | 什么时候定 |
| --- | --- | --- |
| **系统简介** | 3–5 句：这是什么 / 给谁用 / **参考谁 + 简化了什么** / 由哪几个端组成 | Step 1 澄清后即可写 |
| **设计与交互** | 按端各一段（整体布局 / 色彩 / 核心页面 / **一句话**）+ 一段通用交互 | Step 3 出设计系统后写，是它的自然产物 |
| **产品边界** | 逐条列明确排除的功能，每条写明「后续版本迭代」 | **Step 2 骨架确认时就要问定**，别拖到最后 |

「产品边界」是范围边界，**不是免责声明**：没写进来的争议最后都会变成返工。
常见该列而没列的：密码找回、账号注销、举报、意见反馈与客服、真实支付（只留记录与统计）、某类文件格式、某类通知。
与附录E「待确认项清单」分工：**本节是已经决定不做的**，附录E 是**还没定的**。
...
**形态特有的全局章节由 profile 规定**——后台是权限模型与操作日志，APP 是权限申请/推送/版本更新/离线，小程序是授权登录/订阅消息/审核合规，官网是 SEO/CMS 可维护性。**五个形态现在都带「多语言（如有）」与「产品边界」**，多语言要写清覆盖范围（通常只覆盖界面固定文本，不含用户产生的内容）与配置形式。
