...
页面清单统一给一张表作为后续章节目录，**每页带优先级**（🔴 核心 / 🟡 重要 / ⚪ 未来）：

| 编号 | 页面 | 优先级 | 类型 | 入口 | 关键交互 | 关联浮层 |
| --- | --- | --- | --- | --- | --- | --- |
| 7.1 | 内容详情页 | 🔴 | 详情页 | 首页Tab → 卡片点击 | 上滑加载下一条、双击点赞 | 分享面板、举报弹窗 |
| 7.5 | 历史记录页 | 🟡 | 列表页 | 我的 → 最近阅读 | 分页加载 | — |

页面类型**由 profile 规定**，不要跨形态混用。编号从 **7** 起（1–6 是开篇六章，见 Step 5）。

**本步还要定下开篇 3 与 5 两章**：产品边界（尤其 3.4 不包含功能、3.5 核心用户动线）
与功能清单优先级，都在骨架确认这一轮问定，不要拖到最后——它们决定后面写多少页。
...
**开篇六章（落在文档最前面，功能域之前，缺一不可）**——写法与判据见 `references/doc-style.md`「开篇六章」：

| 章 | 写什么 | 什么时候定 | 上游真源（有就摘录，别重做） |
| --- | --- | --- | --- |
| 1 项目背景与目标 | 痛点表 + 短/中/长三档目标 + 愿景一句话 + 形态选型（T16） | Step 1 澄清后 | 阶段0 市场调研 |
| 2 产品定位 | 商业价值 / 盈利模式 / 差异化定位 | Step 1 澄清后 | 阶段-2 战略、阶段-1 GTM |
| 3 产品边界 | 功能/技术/合规三类边界 + 不包含功能（T14）+ **核心用户动线**（Mermaid） | **Step 2 骨架确认时问定** | 阶段4 需求澄清 |
| 4 用户角色与画像 | 角色定义（T15）+ 画像摘要 + 典型场景 | Step 1 澄清后 | 阶段1 用户画像 |
| 5 功能清单与优先级 | 🔴🟡⚪ 树 + 优先级说明 + 章节映射 | **Step 2 骨架确认时问定** | 阶段2 优先级、阶段3 路线图 |
| 6 全局通用规则 | 九类全局规则（权限/加载/网络异常/一致性/埋点/错误日志/安全隐私/缓存限流/文案） | Step 3–4 之间 | 无上游，本章原创 |

**上游有产出就摘录并给出处，只有缺件时才在 PRD 里补写**，并标注「本文档补写，未经专项调研」。
本技能在 `pm-master` 的阶段5，前面阶段已产出调研、画像、优先级、路线图——开篇六章一半是摘录。

三条最容易漏的：

1. **3.5 核心用户动线要画到异常分支**（连接失败→重试、支付失败→重试、验证失败→回登录）。
   只画一条 happy path 等于没画；多端产品**每端一张**。
2. **5.1 功能清单标到二级模块**，不是只标顶层；**5.3 章节映射**保证清单与正文一一对上。
3. **第 6 章与各页「界面与交互规范」不要重复**：第 6 章定全局一次，页面层只在偏离时写「本页例外：…（原因）」。

**范围闸门（写功能正文前必须过）**：算一句给用户——「本期共 N 个 🔴 模块，先只展开这些；
🟡/⚪ 只在清单留名，是否下一版再写？」**得到答复再动笔**。🟡/⚪ 的模块正文不展开、不配原型图。
优先级来源冲突时**列出冲突请用户裁定**。

**通用**（按需选取，属第 6 章的细化或附录）：
