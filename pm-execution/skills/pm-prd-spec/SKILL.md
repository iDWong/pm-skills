...
| 状态与角色 | 有哪些对象要走状态流转（订单/审核/退款/工单）？系统有几个角色？ | 决定要不要 T14 状态机与 T17 权限矩阵——**这两样漏问，写到一半才发现要返工** |
| 触达与依赖 | 哪些动作要通知用户（短信/站内信/推送）？依赖哪些三方服务？ | 决定 T15 通知清单与 T16 外部依赖；三方依赖还会影响技术评审能否通过 |
...
**通用**（按需选取；后四项带触发条件，命中就必须写，没命中不用凑）：

- **业务对象状态机**（`doc-style.md` T14）——**对象有状态且会流转就必须写**（订单/审核/退款/工单/提现/内容发布）。
  一个对象一张表，放在该对象所属功能域的开头，不要塞进某个页面：状态流转横跨多个页面。
  纯 CRUD 对象（会员、标签、配置）不写。
- **通知与消息清单**（T15）——**系统会主动给人发消息就必须写**（短信/站内信/推送/邮件）。
  后台形态最容易漏：后台的操作触发的是发给 C 端用户的消息。
- **外部依赖与降级**（T16）——**依赖三方服务就必须写**（支付/短信/OCR/实名/对象存储）。
  与 T9 的分工：T9 是页面级「这个操作失败了怎么办」，T16 是项目级「我们依赖了谁、挂了怎么办」。
- **权限矩阵**（T17）——**角色 ≥2 个就必须写**。T10 是概念三层模型，T17 是可直接落库的矩阵；
  两者角色名必须逐字一致。
...
| `{版本号}` | 首版 `V1.0`；评审后修订 `V1.1`，结构性重写 `V2.0`。**改了内容就得改文件名**：一律就地 `Edit` + `mv` 改名，**目录里只留最高版本那一份**，不要另存新版留档（见下） |
...
**修订：就地改也要改文件名。** 大文档（几千行 / MB 级）**就地 `Edit` 改**，别整份重写；但改完必须三样一起动——文首「文档版本」、版本记录表、**`mv` 把文件名的版本号也改掉**（局部修订 `+0.1`，结构性重写进大版本；日期取改动当天）。**绝不允许内容已是 V1.1、文件名还写 V1.0。**改名后 `grep` 一遍旧名，把 README 清单、下游「来源」行、`tools/` 脚本里的引用一并改掉。完整规则见 `../common/README.md`。
