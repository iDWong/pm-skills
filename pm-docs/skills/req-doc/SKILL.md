...
**唯一免问情形**：用户本轮原话说过要 Word / 要 docx / 要评审稿文件，或直接以「导出需求说明书」「需求说明书导出Word」「SRS导出」触发本技能——视为已确认，直接导，不再重复问。

**禁止**：把导出当默认收尾动作；先导完再问；用"顺手导了一份"代替确认。

**与 PRD 是两次独立确认**：由 `pm-prd-spec` 出 PRD 后再走 Step F 转 SRS 时，用户在 PRD 那步说过"导出"不代表 SRS 也要导，反之亦然——各自问一次。
...
| 谁写 | 文件名 | 内容 |
| --- | --- | --- |
| 本技能（SRS 链路） | 项目根 `README-SRS.md` | SRS 清单（文档名 / 覆盖模块 / 版本）、章节与规格真源说明、图表来源（`diagram-generator` 产物与 draw.io 源）、与 PRD 的对应关系、Word 导出状态 |
