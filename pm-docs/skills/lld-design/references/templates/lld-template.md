...
> **图片落盘目录**：PNG 必须渲到 **`docs/02-架构与设计/images/`**（与本说明书同级），文档里引用写 `images/xxx.png`。渲到 `docs/images/` 会导致 Word 导出丢图（实测）。调 `diagram-generator` 时直接把输出路径指到 `docs/02-架构与设计/images/`。
