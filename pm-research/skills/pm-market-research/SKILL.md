...
> 流程内落盘路径 `prd/research/market-research-{产品名}-{日期}.md`（流程门禁按 glob 匹配，命名带产品名/日期是正常的）。
> 跑过阶段-1 时只在 `prd/strategy/gtm.md` 定的滩头细分内做聚焦调研，不重做全市场扫描。
...
将完整报告保存至 `prd/research/market-research-[产品名]-[日期].md`
