...
阶段 7–10 与 `dev-master` 阶段 9/11/12 用的是**同一个技能**，但产出**不是同一份东西**，
落盘也各归各根（产品侧 `prd/`、研发侧 `dev/`）。
**看到对方那份：不要删、不要覆盖，也不要因为它存在就认为自己这一步已经做过。**

| 产物 | 本流程（`prd/`） | `dev-master`（`dev/`） |
|---|---|---|
| 测试用例 | 阶段7 `prd/test/`——从 `SPEC_SOURCE` 推导的**验收用例，未执行**，回答「该验哪些」，给评审与客户验收看 | 阶段9 `dev/test/`——用例 + `webapp-testing` **真跑过**的执行报告与缺陷清单，带截图证据，回答「实际跑通没有」 |
| 操作手册 | 阶段8 `prd/release/`——照需求文档与设计稿写，**代码还没出来也能写**，面向最终用户 | 阶段12 `dev/release/`——照**真实实现**写，另含发布与回滚预案，面向运维与发布 |
| 发版说明 | 阶段9 `prd/release/`——按路线图讲**这一版给用户带来什么** | 阶段12 `dev/release/`——随 `release-rollout` / `finishing-branch` 出的**变更与回滚**记录 |
| 上线审计 | 阶段10 `prd/reports/` | 阶段11 `dev/reports/` —— **这两个是同一件事**，见下 |

**上线审计只跑一次。** 两边都是 `pm-ai-ship-audit`、硬输入都是 `dev/code/`、产出也一样，
真实差别只有落盘目录。本流程不出代码，代码只可能来自 `dev-master` 或别处——
**`dev/reports/` 下已有审计报告时，阶段10 直接读它并在交接摘要里引用，不要重跑一份到 `prd/reports/`。**

前三类则**不要**因为对方已有就跳过：文档态的用例与实现态的报告互相替代不了，
...
| `common/export-word.*` | 文档类阶段要交 Word（阶段 -2/0/5/7/8，以及 `feasibility-report`） |
