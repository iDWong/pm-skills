...
- 已有市场调研报告（从阶段 1 开始）
- 已有明确需求（从阶段 4 开始）
- 已有 SRS 或 PRD（**阶段 5 不跳过，退化为「校验 + 登记 `SPEC_SOURCE`」**，然后进阶段 6；要把它做出来 → 转 `dev-master`）
...
1. **本阶段输入块列出的文件路径**（见 `stages/s<N>-*.md` 的「输入」）——以这些为准，技能侧不要另行搜索
2. **上一阶段的交接摘要**（≤10 行）——已有的上下文不要重新扫描
3. **Step 0 已收集的信息**：裁剪范围、档位、`DELIVERY_MODE`、阶段5 文档类型

> ⚠️ **输入块里的路径一律是 glob，不是真实文件名**（各阶段技能的实际命名带产品名／日期／版本号，
> 理由同下方「阶段间传递规范」）。**解析是流程的活，不是技能的活**：调用前流程先把 glob 解析成
> 真实路径（命中多个取最新修改的那个），再把**解析后的绝对／相对路径**传进去；技能拿到的永远是
> 确定的文件名。传入上下文模板里的 `{…}` 占位符就是等着填这个解析结果的，不要把 glob 原样传下去。

> 约束原话（调用时带上）：「本次由 pm-master 流程调用。输入以上述已解析的路径为准，不要用宽 glob 自行搜索项目；
...
| 7 | `prd/test/*测试用例*.md` 或 `prd/test/*test-case*.md` | 8 |
...
| 阶段7 测试用例 | 必须确认 | 摘要后默认继续 | 默认继续 |
| 阶段8 操作手册 | 必须确认 | 摘要后默认继续 | 默认继续 |
| 阶段9 发版说明 | 必须确认 | 摘要后默认继续 | 默认继续 |
| 阶段10 审计发现 | 必须确认 | 报告写完告知路径，默认继续 | 同标准 |

**表里没列的阶段（1 用户画像、2 功能优先级）**：一律「摘要后默认继续」，严格模式下才停。
新增阶段时**必须往这张表里加一行**，不要靠这条兜底。
