...
| # | 问题 | 不通过的典型形状 |
| --- | --- | --- |
| **Q1 可进入** | 点它有没有东西出现／变化？ | 点了没反应；弹窗被状态帧挡住打不开；筛选项是不可点的文本而非按钮 |
...
设计稿常把同一页的多种数据状态做成可切换的"出稿状态"。**这两类判据完全不同**：

| 类型 | 定义 | 判据 |
| --- | --- | --- |
| **流程状态** | 由用户操作推进（第 1 步→第 2 步、未签到→已签到、呼叫中→通话中→已挂断） | **必须**能从界面上点到；只能靠工具条切 = 不通过 |
| **页签/筛选状态** | 由用户选择切换（全部/充值/消费、剧场/片库、四个面板） | **必须**能点且**必须真的换内容**；只换高亮不换内容 = 不通过 |
| **数据状态** | 由真实数据决定（离线、空态、加载中、失败、未成年、额度用完、已下架、审核中） | **允许**只由工具条切；把「离线态」做成一个能点出来的按钮反而是错的 |

**判定口诀**：问一句「真实用户能不能靠自己的操作到达这个状态？」——能，就必须点得到；不能，就归数据状态。
