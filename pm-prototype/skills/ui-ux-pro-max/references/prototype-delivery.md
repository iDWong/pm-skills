...
1. **必须先读 PRD 与 SRS 全文再动手**。禁止只读目录、只读章节标题、只读其中一份就开始写页面。
   页面清单、字段、枚举、文案原文、校验规则、状态机、**权限矩阵**、指标口径**一律照抄文档**，不得自创。
   ...
   **角色与权限从哪来**：本契约多处要求做权限相关的产物——权限隐藏态、「无权限」演示数据、
   登录页给不同角色各一个演示账号、三态权限层各给各的出路。**这些的唯一依据是文档里的权限矩阵**
   （PRD 侧见 `pm-prd-spec` 的 T17「权限矩阵」与 T10「权限模型表」；SRS 侧在 3.4 页面访问权限 + 2.5 业务角色定义），
   **不要自己拟角色、自己猜谁看得到什么**。三件事照抄：
   ① **角色清单**（登录页的演示账号按它一一对应，名称逐字一致）；
   ② **功能·操作粒度的可见性**（同一页的「查看」与「导出」往往不同权限，别按页面整体处理）；
   ③ **无权限的表现**——文档写「不渲染」就整个按钮不出现，写「置灰 + tooltip」才做灰态。
   **这两种是不同的产品决策，不能互换**：置灰会暴露功能存在，敏感/资金类操作通常要求不渲染。
   文档里没有权限矩阵而页面又明显分角色时，**回去问，不要自己编一套**。

   **状态机同理**：PRD 的状态机表（`pm-prd-spec` T14）决定 `FLOWS.md` 里哪些分支必须走得到——
   每条状态流转都该是一条可点通的流程，「此状态下不允许的操作」则对应页面上该按钮的禁用或不出现。
